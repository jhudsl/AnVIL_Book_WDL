...
1. Understand how to specify a container for a Terra workflow
...
```{r, echo=FALSE}
ottrpal::include_slide("https://docs.google.com/presentation/d/1o2XnuMbqWVLf4XrsXolIQ7ulfnMlpJlrUxN0Y8aLIVQ/edit#slide=id.g278ce060607_0_16")
```
...
The next step is to write a WDL script that will run in a container built by this Docker image. In this exercise, our WDL script will calculate QC metrics for a BAM file using the `samtools idxstats` function. 

Modify the template script below to call `samtools idxstats` on a bam file – you’ll have to add the task’s name (in two places), the inputs and outputs, the task commands, and the docker image:
...
Then, compare your version to the completed version below. Note that your version should point to the name of your own Docker image’s repository, rather than `cutsort/test`.
...
Once you’ve exported the workflow to your Terra workspace, open the workflow. 
...
- [Docker/container overview](https://support.terra.bio/hc/en-us/articles/360037340472-Docker-container-overview)
- [How to install docker and test that it works](https://support.terra.bio/hc/en-us/articles/360036000631-How-to-install-Docker-and-test-that-it-works)
